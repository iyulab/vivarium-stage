...
| `prepare(branch, facetOps)` | Stage the changeset's operations (logical schema ops, data ops, UI artifact payloads) into the branch, in the order the changeset spec §5.4 fixes: additive schema operations, then data operations, then removing ones — and a removal takes the stored values with it. Idempotent per changeset fingerprint; MUST report per-facet completion **for exactly the facets the document carried** — a facet the document did not carry is not work this prepare completed, and claiming it is completion for work not done. No live effect. |
